<template>
  <div>
    ...
    <div class="mb-8 text-lg">Vote Result from User : {{ $route.params.id }}</div>
    ...
    <template #thead="{sorting, sort}">
      ...
      <table-head
        ...
        :sort="sort"
        multiple
        @sorting="sorting">Score</table-head>
      <!-- <table-head>Action</table-head> -->
    </template>

    <template #tbody="{row}">
      <table-body v-text="row.name"/>
      <table-body v-text="row.score"/>
...
const Page = defineComponent({
  ...
  data() {
    return {
      breadcrumbData: [
        ...
        { name: 'User Votes', href: `/users/${this.$route.params.id}/vote-status`, current: true },
      ],
      tableData: {
        data: [],
        meta: {}
  ...
  methods: {
    loadData(query) {
      ...
      // dummy
      const arr = [
        {name: 'Keluarga Cemara', score: 70},
        {name: 'Film 2', score: 50},
        {name: 'Film 3', score: 80},
        {name: 'Film 4', score: 55},
        {name: 'Film 5', score: 95},
      ]
